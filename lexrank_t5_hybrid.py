...
1. Extractive summaries - selects important sentences from the original text using LexRank
2. Abstractive summaries - generates new text that captures the essence of the article using T5
...
class RSSFeedSummarizer:
    ...
    def lexrank_summarization(self, text, num_sentences=5):
        ...
        LexRank is a graph-based extractive summarization algorithm that uses cosine similarity
        and tf-idf to rank sentences by importance.
        """
        ...
        # Get similarities between sentences using TF-IDF
        similarity_matrix = tfidf_matrix * tfidf_matrix.T
        similarity_matrix = similarity_matrix.toarray()

        # Normalize the matrix - this is a key component of LexRank
        for i in range(len(similarity_matrix)):
            for j in range(len(similarity_matrix)):
                if i != j:
                    # Apply threshold to create a modified adjacency matrix
                    # Typical LexRank uses a threshold, but here we'll use the raw similarity
                    if similarity_matrix[i][j] < 0.1:  # Threshold value
                        similarity_matrix[i][j] = 0

        # Generate graph and apply PageRank
        nx_graph = nx.from_numpy_array(similarity_matrix)
        # Alpha is the damping factor in PageRank
    ...
    def t5_summarization(self, text, max_length=150, min_length=50):
        ...
        T5 (Text-To-Text Transfer Transformer) is trained specifically for text-to-text tasks
        and provides good abstractive summarization capabilities
        """
        # Preprocess text for T5 - it requires a specific prefix for summarization
    ...
    def hybrid_summarization(self, text, original_text, extractive_summary, abstractive_summary):
        """
        Generate a hybrid summary that combines LexRank and T5 approaches
    ...
    def process_feed(self, rss_url, num_articles=3):
        ...
        for i, article in enumerate(articles):
            ...
            # Generate summaries
            print("  Generating extractive summary using LexRank...")
            extractive_summary = self.lexrank_summarization(content)

            print("  Generating abstractive summary using T5...")
            abstractive_summary = self.t5_summarization(content)

            print("  Generating hybrid summary (LexRank + T5)...")
            ...
            print(
                f"  Extractive summary (LexRank): {len(extractive_summary.split())} words")
            print(
                f"  Abstractive summary (T5): {len(abstractive_summary.split())} words")
            print(
                f"  Hybrid summary (LexRank + T5): {len(hybrid_summary.split())} words")

            print("\n  --- ROUGE Scores ---")
            print(
                f"  Extractive (LexRank) - ROUGE-1: {rouge_extractive['rouge-1']['f']:.4f}, ROUGE-2: {rouge_extractive['rouge-2']['f']:.4f}, ROUGE-L: {rouge_extractive['rouge-l']['f']:.4f}")
            print(
                f"  Abstractive (T5) - ROUGE-1: {rouge_abstractive['rouge-1']['f']:.4f}, ROUGE-2: {rouge_abstractive['rouge-2']['f']:.4f}, ROUGE-L: {rouge_abstractive['rouge-l']['f']:.4f}")
            print(
                f"  Hybrid (LexRank + T5) - ROUGE-1: {rouge_hybrid['rouge-1']['f']:.4f}, ROUGE-2: {rouge_hybrid['rouge-2']['f']:.4f}, ROUGE-L: {rouge_hybrid['rouge-l']['f']:.4f}")

            print("\n  --- BERTScore ---")
            print(
                f"  Extractive (LexRank) - F1: {bertscore_extractive['f1']:.4f}")
            print(
                f"  Abstractive (T5) - F1: {bertscore_abstractive['f1']:.4f}")
            print(
                f"  Hybrid (LexRank + T5) - F1: {bertscore_hybrid['f1']:.4f}")
    ...
    def _calculate_average_scores(self, results, num_articles):
        ...
        for method, label in zip(['extractive', 'abstractive', 'hybrid'],
                                 ['Extractive (LexRank)', 'Abstractive (T5)', 'Hybrid (LexRank + T5)']):
            print(
    ...
    def _visualize_results(self, results):
        ...
        data = {
            'Method': ['Extractive (LexRank)', 'Abstractive (T5)', 'Hybrid (LexRank+T5)'],
        ...
        plt.figtext(0.5, 0.01,
                    'LexRank: Graph-based extractive algorithm using TF-IDF and PageRank\n'
                    'T5: Pre-trained encoder-decoder transformer model for text-to-text generation\n'
                    'Hybrid: Adaptive combination of LexRank and T5 to maximize evaluation scores',
                    ha='center', fontsize=11, bbox={"facecolor": "lightgrey", "alpha": 0.5, "pad": 5})

        plt.tight_layout(rect=[0, 0.05, 1, 0.95])
        plt.savefig('lexrank_t5_hybrid_results.png',
                    dpi=300, bbox_inches='tight')
        plt.close()

        print("\nEnhanced visualization with algorithm/model labels saved as 'lexrank_t5_hybrid_results.png'")
...
def main():
    ...
    print("Initializing RSS Feed Summarizer with LexRank and T5...")
